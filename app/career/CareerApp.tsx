...
import {
  Activity, Archive, ArrowRight, ArrowUpRight, BarChart3, Bell,
  ...
  MoreHorizontal, PanelTop, Pencil, Plus, RotateCcw, Search, Settings,
...
export default function CareerApp() {
  ...
  function navigate(next: CareerView) { setView(next); setSidebarOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); }
  ...
  return <main className="career-app">
    ...
    <section className="career-main">
      <Topbar title={navItems.find((item) => item.id === view)?.label ?? "职迹"} query={query} onQuery={setQuery} onSearch={() => setSearchOpen(true)} onMenu={() => setSidebarOpen(true)} onAdd={() => setModal("job")} />
      <div className="career-content">
        ...
        {view === "materials" && <MaterialsView data={data} onAdd={() => setModal("material")} onAi={runAi} />}
        ...
        {view === "settings" && <SettingsView data={data} onRefresh={refresh} onExport={async () => {
          try {
            ...
            anchor.href = url; anchor.download = `zhiji-${new Date().toISOString().slice(0, 10)}.sqlite3`; anchor.click(); URL.revokeObjectURL(url); notify("完整备份已导出");
          } catch (error) { notify(error instanceof Error ? error.message : "导出失败", "error"); }
        }} onImport={async (file) => {
          if (!window.confirm("恢复备份会替换当前职迹数据库。确定继续吗？")) return;
          try { await importCareerDb(new Uint8Array(await file.arrayBuffer())); await initializeCareerDb(); await refresh(); notify("备份已恢复"); }
...
function Sidebar({ view, open, data, onNavigate, onClose }: { view: CareerView; open: boolean; data: CareerData; onNavigate: (view: CareerView) => void; onClose: () => void }) {
  ...
  const completedThisWeek = data.jobs.filter((job) => job.applied_at && CAREER_CLOCK - new Date(job.applied_at).getTime() < 7 * 86_400_000).length;
  return <><button className={`career-scrim ${open ? "show" : ""}`} aria-label="关闭导航" onClick={onClose} /><aside className={`career-sidebar ${open ? "open" : ""}`}>
    ...
    <nav className="career-nav" aria-label="职迹主导航">{navItems.map((item) => <button key={item.id} className={view === item.id ? "active" : ""} onClick={() => onNavigate(item.id)}><item.icon size={18} strokeWidth={1.8} /><span>{item.label}</span>{item.id === "calendar" && data.tasks.filter((task) => task.status === "todo").length > 0 && <em>{data.tasks.filter((task) => task.status === "todo").length}</em>}</button>)}</nav>
    <div className="career-sidebar-spacer" /><div className="career-goal-card"><div><span>本周投递</span><b>{completedThisWeek}<small> / 8</small></b></div><div className="career-progress"><i style={{ width: `${Math.min(100, completedThisWeek / 8 * 100)}%` }} /></div><p>{active} 个机会正在推进，保持节奏。</p></div><div className="career-privacy"><ShieldCheck size={15} /><span>资料保存在本地 SQLite</span><i /></div>
  </aside></>;
}

function Topbar({ title, query, onQuery, onSearch, onMenu, onAdd }: { title: string; query: string; onQuery: (value: string) => void; onSearch: () => void; onMenu: () => void; onAdd: () => void }) {
  return <header className="career-topbar"><div className="career-topbar-title"><button className="career-icon-button mobile-only" onClick={onMenu} aria-label="打开导航"><Menu size={20} /></button><h1>{title}</h1></div><div className="career-topbar-actions"><label className="career-search"><Search size={16} /><input value={query} onChange={(event) => onQuery(event.target.value)} placeholder="搜索职位、公司、标签" aria-label="搜索" /><kbd>⌘ K</kbd></label><button className="career-icon-button command-compact" onClick={onSearch} aria-label="打开搜索"><Search size={18} /></button><button className="career-button primary" onClick={onAdd}><Plus size={17} />记录职位</button><button className="career-avatar" aria-label="个人设置">FK<span /></button></div></header>;
}
...
function TodayView({ data, onNavigate, onSelectJob, onToggleTask, onAddJob, onAi }: { data: CareerData; onNavigate: (view: CareerView) => void; onSelectJob: (id: string) => void; onToggleTask: (task: Task) => void; onAddJob: () => void; onAi: (action: AiAction, title: string, payload: unknown) => void }) {
  ...
  const upcoming = data.interviews.filter((item) => item.status !== "completed" && item.scheduled_at && new Date(item.scheduled_at).getTime() > CAREER_CLOCK).sort((a, b) => (a.scheduled_at ?? "").localeCompare(b.scheduled_at ?? ""));
  const activeJobs = data.jobs.filter((job) => !["stage_accepted", "stage_rejected", "stage_withdrawn"].includes(job.stage_id));
  const applied = data.jobs.filter((job) => job.applied_at);
  const responsive = data.jobs.filter((job) => ["stage_assessment", "stage_interview", "stage_offer", "stage_accepted"].includes(job.stage_id));
  ...
  return <div className="career-view career-today"><section className="career-welcome"><div><span>{date}</span><h2>下午好，今天把最重要的一步走好。</h2><p>你的资料已在本地准备好。这里是下一步最值得关注的事项。</p></div><div className="career-welcome-actions"><button className="career-button secondary" onClick={() => onNavigate("calendar")}><CalendarDays size={16} />查看日程</button><button className="career-button primary" onClick={onAddJob}><Plus size={16} />记录职位</button></div></section>
    <section className="career-metric-grid"><Metric label="活跃机会" value={String(activeJobs.length)} note={`${data.jobs.filter((job) => job.stage_id === "stage_interview").length} 个正在面试`} icon={<BriefcaseBusiness size={18} />} /><Metric label="待办事项" value={String(openTasks.length)} note={`${openTasks.filter((task) => task.due_at && new Date(task.due_at).getTime() < CAREER_CLOCK).length} 项已逾期`} icon={<ListTodo size={18} />} tone="amber" /><Metric label="近期待面" value={String(upcoming.length)} note={upcoming[0] ? `${formatDate(upcoming[0].scheduled_at, true)} · ${upcoming[0].round_name}` : "暂未安排"} icon={<CalendarDays size={18} />} tone="plum" /><Metric label="有效回复率" value={`${applied.length ? Math.round(responsive.length / applied.length * 100) : 0}%`} note={`${responsive.length} / ${applied.length || 0} 次投递已推进`} icon={<Activity size={18} />} tone="green" /></section>
    ...
    </section><section className="career-panel career-recent"><SectionHeading title="最近动态" description="所有推进都自动留在时间线里" action={<button className="career-text-button" onClick={() => onNavigate("jobs")}>查看全部</button>} /><div className="career-activity-row">{data.activities.slice(0, 4).map((item) => { const job = data.jobs.find((entry) => entry.id === item.job_id); return <button key={item.id} onClick={() => job && onSelectJob(job.id)}><span className={`career-activity-icon ${item.type}`}><Zap size={15} /></span><span><b>{item.detail}</b><small>{job ? `${job.company} · ${job.role}` : "职迹"}</small></span><time>{formatDate(item.created_at)}</time></button>; })}</div></section>
  </div>;
}

type SensorValue = ReturnType<typeof useSensors>;
function BoardView({ data, jobs, query, sourceFilter, priorityOnly, onSourceFilter, onPriorityOnly, onClear, onSelectJob, onAddJob, sensors, activeJob, onDragStart, onDragEnd }: { data: CareerData; jobs: Job[]; query: string; sourceFilter: string; priorityOnly: boolean; onSourceFilter: (value: string) => void; onPriorityOnly: (value: boolean) => void; onClear: () => void; onSelectJob: (id: string) => void; onAddJob: () => void; sensors: SensorValue; activeJob: Job | null; onDragStart: (event: DragStartEvent) => void; onDragEnd: (event: DragEndEvent) => void }) {
  const stages = data.stages.filter((stage) => !stage.hidden && !["stage_accepted", "stage_rejected", "stage_withdrawn"].includes(stage.id));
  return <div className="career-view career-board-view"><SectionHeading eyebrow="PIPELINE" title="求职看板" description={`${jobs.length} 个机会 · 拖动卡片更新进度`} action={<div className="career-view-actions"><button className={`career-chip ${priorityOnly ? "active" : ""}`} onClick={() => onPriorityOnly(!priorityOnly)}><Target size={14} />只看重点</button><select className="career-select compact" value={sourceFilter} onChange={(event) => onSourceFilter(event.target.value)} aria-label="来源筛选"><option value="all">全部来源</option>{[...new Set(data.jobs.map((job) => job.source))].map((source) => <option key={source}>{source}</option>)}</select><button className="career-icon-button" onClick={onClear} aria-label="清除筛选"><RotateCcw size={16} /></button></div>} />{(query || sourceFilter !== "all" || priorityOnly) && <div className="career-filter-summary"><Filter size={14} />当前显示 {jobs.length} 个结果<button onClick={onClear}>清除全部</button></div>}
    <DndContext sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd}><div className="career-board-scroll">{stages.map((stage) => <BoardColumn key={stage.id} stage={stage} jobs={jobs.filter((job) => job.stage_id === stage.id)} data={data} onSelectJob={onSelectJob} onAddJob={onAddJob} />)}</div><DragOverlay>{activeJob ? <JobCard job={activeJob} data={data} overlay onSelect={() => undefined} /> : null}</DragOverlay></DndContext>
  </div>;
}

function BoardColumn({ stage, jobs, data, onSelectJob, onAddJob }: { stage: Stage; jobs: Job[]; data: CareerData; onSelectJob: (id: string) => void; onAddJob: () => void }) {
  const { isOver, setNodeRef } = useDroppable({ id: `stage:${stage.id}` });
  return <section ref={setNodeRef} className={`career-board-column ${isOver ? "over" : ""}`}><header><div><i style={{ background: stage.color }} /><b>{stage.name}</b><span>{jobs.length}</span></div><button onClick={onAddJob} aria-label={`在${stage.name}添加职位`}><Plus size={16} /></button></header><div className="career-board-cards">{jobs.map((job) => <DraggableJobCard key={job.id} job={job} data={data} onSelect={() => onSelectJob(job.id)} />)}{jobs.length === 0 && <div className="career-board-empty">拖到这里</div>}</div><button className="career-add-inline" onClick={onAddJob}><Plus size={15} />添加职位</button></section>;
}

function DraggableJobCard({ job, data, onSelect }: { job: Job; data: CareerData; onSelect: () => void }) {
  const { attributes, listeners, setNodeRef, transform, isDragging } = useDraggable({ id: job.id });
  const style = transform ? { transform: `translate3d(${transform.x}px, ${transform.y}px, 0)` } : undefined;
  return <div ref={setNodeRef} style={style} className={isDragging ? "dragging" : ""}><JobCard job={job} data={data} onSelect={onSelect} dragProps={{ ...attributes, ...listeners }} /></div>;
}

function JobCard({ job, data, onSelect, overlay = false, dragProps = {} }: { job: Job; data: CareerData; onSelect: () => void; overlay?: boolean; dragProps?: Record<string, unknown> }) {
  const nextTask = data.tasks.find((task) => task.job_id === job.id && task.status === "todo");
  const upcoming = data.interviews.find((item) => item.job_id === job.id && item.status !== "completed");
  return <div className={`career-job-card ${overlay ? "overlay" : ""}`} role="button" tabIndex={0} onClick={onSelect} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(); } }}><div className="career-job-card-top"><CompanyMark company={job.company} small /><button className="career-grip" {...dragProps} onClick={(event) => event.stopPropagation()} aria-label="拖动职位"><GripVertical size={16} /></button></div><h3>{job.role}</h3><p>{job.company}</p><div className="career-card-meta"><span>{job.location || "地点待定"}</span>{job.work_mode && <span>{job.work_mode}</span>}</div><div className="career-card-tags">{job.tags.split(",").filter(Boolean).slice(0, 2).map((tag) => <i key={tag}>{tag}</i>)}</div><div className={`career-next ${nextTask?.due_at && new Date(nextTask.due_at).getTime() < CAREER_CLOCK ? "late" : ""}`}><Clock3 size={13} /><span>{upcoming ? `${formatDate(upcoming.scheduled_at, true)} · ${upcoming.round_name}` : nextTask ? `${relativeDate(nextTask.due_at)} · ${nextTask.title}` : "还没有下一步"}</span></div><footer><SourceBadge source={job.source} /><span className="career-priority" aria-label={`优先级 ${job.priority}`}>{[1, 2, 3].map((dot) => <i key={dot} className={dot <= job.priority ? "active" : ""} />)}</span></footer></div>;
}
...
function JobsView({ data, jobs, stageFilter, sourceFilter, priorityOnly, onStageFilter, onSourceFilter, onPriorityOnly, onSelectJob, onImport }: { data: CareerData; jobs: Job[]; stageFilter: string; sourceFilter: string; priorityOnly: boolean; onStageFilter: (value: string) => void; onSourceFilter: (value: string) => void; onPriorityOnly: (value: boolean) => void; onSelectJob: (id: string) => void; onImport: () => void }) {
  return <div className="career-view"><SectionHeading eyebrow="APPLICATIONS" title="全部职位" description={`共 ${data.jobs.length} 个职位，${data.jobs.filter((job) => job.stage_id === "stage_interview").length} 个正在面试`} action={<button className="career-button secondary" onClick={onImport}><Import size={16} />智能导入</button>} />
    <div className="career-toolbar"><select className="career-select" value={stageFilter} onChange={(event) => onStageFilter(event.target.value)}><option value="all">全部阶段</option>{data.stages.map((stage) => <option key={stage.id} value={stage.id}>{stage.name}</option>)}</select><select className="career-select" value={sourceFilter} onChange={(event) => onSourceFilter(event.target.value)}><option value="all">全部来源</option>{[...new Set(data.jobs.map((job) => job.source))].map((source) => <option key={source}>{source}</option>)}</select><button className={`career-chip ${priorityOnly ? "active" : ""}`} onClick={() => onPriorityOnly(!priorityOnly)}><Target size={14} />高优先级</button><span className="career-toolbar-count">显示 {jobs.length} 条</span></div>
...
function InterviewsView({ data, onAdd, onSelect, onAi }: { data: CareerData; onAdd: () => void; onSelect: (id: string) => void; onAi: (action: AiAction, title: string, payload: unknown) => void }) {
  ...
  const shown = data.interviews.filter((item) => tab === "archive" ? item.status === "completed" : item.status !== "completed");
  return <div className="career-view"><SectionHeading eyebrow="INTERVIEW LOG" title="面试与面经" description="每一轮都有准备、有记录，也有下一次会用到的经验" action={<button className="career-button primary" onClick={onAdd}><Plus size={16} />安排面试</button>} /><div className="career-segmented"><button className={tab === "upcoming" ? "active" : ""} onClick={() => setTab("upcoming")}>即将进行</button><button className={tab === "archive" ? "active" : ""} onClick={() => setTab("archive")}>面经档案</button></div><div className="career-interview-grid">{shown.map((item) => { const job = data.jobs.find((entry) => entry.id === item.job_id); const questions = parseQuestions(item.questions_json); return <div className="career-interview-card" key={item.id} role="button" tabIndex={0} onClick={() => onSelect(item.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(item.id); } }}><header><CompanyMark company={job?.company ?? "职"} /><div><span>{item.status === "completed" ? "已完成" : relativeDate(item.scheduled_at)}</span><h3>{job?.company} · {item.round_name}</h3><p>{job?.role}</p></div><button className="career-icon-button" onClick={(event) => { event.stopPropagation(); onSelect(item.id); }} aria-label="打开面经"><ArrowUpRight size={17} /></button></header><div className="career-interview-meta"><span><CalendarDays size={14} />{formatDate(item.scheduled_at, true)}</span><span><Clock3 size={14} />{item.duration} 分钟</span><span><UserRound size={14} />{item.interviewer || "面试官待确认"}</span></div>{item.status === "completed" ? <div className="career-experience-preview"><p>{item.summary || "还没有写面试总结。"}</p><footer><span>{questions.length} 个问题</span><span>{item.reflection ? "已完成复盘" : "待复盘"}</span></footer></div> : <div className="career-interview-actions"><button className="career-button secondary" onClick={(event) => { event.stopPropagation(); onAi("interview_prep", "生成面试准备包", { job, interview: item }); }}><Sparkles size={15} />AI 准备包</button>{safeLink(item.meeting_url) && <a className="career-button ghost" href={item.meeting_url} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>加入会议 <ExternalLink size={14} /></a>}</div>}</div>; })}</div>{shown.length === 0 && <EmptyState icon={<MessageSquareText />} title={tab === "archive" ? "还没有面经" : "暂未安排面试"} text="记录每一轮问题、回答和复盘，让经验真正沉淀下来。" action={<button className="career-button primary" onClick={onAdd}>安排第一轮</button>} />}</div>;
}

function ContactsView({ data, onAdd, onCreateTask }: { data: CareerData; onAdd: () => void; onCreateTask: (contact: Contact) => void }) {
  return <div className="career-view"><SectionHeading eyebrow="RELATIONSHIPS" title="求职人脉" description="把每次沟通变成清晰、得体的下一步" action={<button className="career-button primary" onClick={onAdd}><Plus size={16} />添加联系人</button>} /><div className="career-contact-grid">{data.contacts.map((contact) => <article className="career-contact-card" key={contact.id}><header><span className="career-contact-avatar">{initials(contact.name)}</span><div><h3>{contact.name}</h3><p>{contact.role} · {contact.company}</p></div><button className="career-icon-button"><MoreHorizontal size={17} /></button></header><div className="career-contact-meta"><span><ContactRound size={14} />{contact.channel}</span><span><Clock3 size={14} />上次联系 {formatDate(contact.last_contact_at)}</span></div><p>{contact.notes || "暂无备注"}</p><footer><span className={contact.next_follow_up && new Date(contact.next_follow_up).getTime() < CAREER_CLOCK ? "late" : ""}>下次跟进：{relativeDate(contact.next_follow_up)}</span><button onClick={() => onCreateTask(contact)}>创建待办 <ArrowRight size={14} /></button></footer></article>)}</div></div>;
}

function MaterialsView({ data, onAdd, onAi }: { data: CareerData; onAdd: () => void; onAi: (action: AiAction, title: string, payload: unknown) => void }) {
  ...
  return <div className="career-view"><SectionHeading eyebrow="MATERIALS" title="求职材料" description="保留每一个版本，也记住哪一份发给了谁" action={<button className="career-button primary" onClick={onAdd}><Plus size={16} />添加材料</button>} /><div className="career-material-hero"><div><Sparkles size={20} /><span><b>让材料更贴合目标职位</b><small>AI 只使用你选择的材料与职位描述，结果保存为草稿。</small></span></div><button className="career-button secondary" onClick={() => onAi("tailor_material", "AI 定制材料", { jobs: data.jobs.filter((job) => job.priority >= 3), materials: data.materials })}>开始定制 <ArrowRight size={15} /></button></div><div className="career-material-grid">{data.materials.map((material) => { const job = data.jobs.find((item) => item.id === material.linked_job_id); return <article className="career-material-card" key={material.id}><span className={`career-file-icon ${material.kind}`}><FileText size={23} /></span><div><header><span>{material.kind}</span><em className={material.status}>{material.status === "sent" ? "已发送" : material.status === "draft" ? "编辑中" : "可使用"}</em>{material.file_key && <em className="attached">本地附件</em>}</header><h3>{material.name}</h3><p>{material.notes}</p><footer><span>{material.version} · 更新于 {formatDate(material.updated_at)}</span>{job && <small>用于 {job.company}</small>}{material.file_name && <small>{material.file_name} · {Math.max(1, Math.round((material.byte_size ?? 0) / 1024))} KB</small>}</footer></div>{material.file_key ? <button className="career-icon-button" onClick={() => void openFile(material.file_key!)} aria-label={`打开 ${material.file_name ?? material.name}`}><Download size={17} /></button> : <button className="career-icon-button" onClick={onAdd} aria-label="添加附件"><MoreHorizontal size={18} /></button>}</article>; })}</div></div>;
}

function AnalyticsView({ data }: { data: CareerData }) {
  const applied = data.jobs.filter((job) => job.applied_at);
  const counts = data.stages.filter((stage) => !stage.hidden).map((stage) => ({ stage, count: data.jobs.filter((job) => job.stage_id === stage.id).length }));
  const max = Math.max(1, ...counts.map((item) => item.count));
  const sources = [...new Set(data.jobs.map((job) => job.source))].map((source) => ({ source, total: data.jobs.filter((job) => job.source === source).length, advanced: data.jobs.filter((job) => job.source === source && ["stage_interview", "stage_offer", "stage_accepted"].includes(job.stage_id)).length })).sort((a, b) => b.total - a.total);
  const interviewRate = applied.length ? Math.round(data.jobs.filter((job) => ["stage_interview", "stage_offer", "stage_accepted"].includes(job.stage_id)).length / applied.length * 100) : 0;
  const offerRate = applied.length ? Math.round(data.jobs.filter((job) => ["stage_offer", "stage_accepted"].includes(job.stage_id)).length / applied.length * 100) : 0;
  return <div className="career-view"><SectionHeading eyebrow="INSIGHTS" title="求职分析" description="用事实看清节奏，不让数字替你定义结果" action={<select className="career-select"><option>最近 90 天</option><option>最近 30 天</option><option>全部时间</option></select>} /><section className="career-metric-grid analytics"><Metric label="累计投递" value={String(applied.length)} note="当前数据库记录" icon={<ArrowUpRight size={18} />} /><Metric label="面试转化" value={`${interviewRate}%`} note={`${data.jobs.filter((job) => job.stage_id === "stage_interview").length} 个正在面试`} icon={<MessageSquareText size={18} />} tone="plum" /><Metric label="Offer 率" value={`${offerRate}%`} note={`${data.jobs.filter((job) => job.stage_id === "stage_offer").length} 个待决定`} icon={<Target size={18} />} tone="green" /><Metric label="待办完成" value={`${data.tasks.length ? Math.round(data.tasks.filter((task) => task.status === "done").length / data.tasks.length * 100) : 0}%`} note={`${data.tasks.filter((task) => task.status === "done").length} / ${data.tasks.length}`} icon={<CheckCircle2 size={18} />} tone="amber" /></section><div className="career-analytics-grid"><section className="career-panel career-funnel"><header><div><h3>机会分布</h3><p>当前职位所在阶段</p></div><span>样本 {data.jobs.length}</span></header><div className="career-bars">{counts.filter((item) => item.count > 0).map((item) => <div key={item.stage.id}><span>{item.stage.name}</span><i><b style={{ width: `${Math.max(7, item.count / max * 100)}%`, background: item.stage.color }} /></i><em>{item.count}</em></div>)}</div></section><section className="career-panel career-source-performance"><header><div><h3>来源效果</h3><p>推进至面试及以后</p></div></header>{sources.map((item) => <div key={item.source}><SourceBadge source={item.source} /><span><b>{item.total}</b> 个机会</span><em>{item.total ? Math.round(item.advanced / item.total * 100) : 0}% 推进</em></div>)}</section></div><section className="career-insight-card"><span><Sparkles size={18} /></span><div><b>本周值得留意</b><p>高优先级职位里，仍有 {data.jobs.filter((job) => job.priority >= 3 && !data.tasks.some((task) => task.job_id === job.id && task.status === "todo")).length} 个没有下一步行动。为它们安排一个具体任务，比继续扩大投递量更有帮助。</p></div></section></div>;
}
...
function ContactModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => Promise<void> }) {
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); await runCareerSql("INSERT INTO career_contacts (id,company,name,role,channel,email,phone,last_contact_at,next_follow_up,notes,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)", [newId("contact"), form.get("company"), form.get("name"), form.get("role"), form.get("channel"), form.get("email"), form.get("phone"), new Date().toISOString(), fromDateInput(String(form.get("next_follow_up") || "")), form.get("notes"), new Date().toISOString()]); await onSaved(); }
  return <Modal title="添加联系人" description="记住关系，也记住下一次联系。" onClose={onClose}><form className="career-form" onSubmit={submit}><div className="career-form-row"><Field label="姓名"><input name="name" required /></Field><Field label="公司"><input name="company" required /></Field></div><div className="career-form-row"><Field label="职位 / 关系"><input name="role" placeholder="Recruiter / 内推人" /></Field><Field label="沟通渠道"><select name="channel"><option>LinkedIn</option><option>BOSS直聘</option><option>邮件</option><option>微信</option><option>电话</option></select></Field></div><div className="career-form-row"><Field label="邮箱"><input name="email" type="email" /></Field><Field label="电话"><input name="phone" /></Field></div><Field label="下次跟进"><input name="next_follow_up" type="datetime-local" /></Field><Field label="备注"><textarea name="notes" rows={4} placeholder="怎么认识、聊过什么、需要跟进什么…" /></Field><div className="career-form-actions"><button type="button" className="career-button ghost" onClick={onClose}>取消</button><button className="career-button primary">保存联系人</button></div></form></Modal>;
}
...
function AiPreview({ state, onClose, onAccept }: { state: { action: AiAction; title: string; loading: boolean; result?: unknown; error?: string }; onClose: () => void; onAccept: () => Promise<void> }) {
  return <Modal title={state.title} description="AI 结果只是一份草稿，接受前请核对事实与措辞。" onClose={onClose} wide><div className="career-ai-preview">{state.loading ? <div className="career-ai-loading"><span><Sparkles size={22} /></span><LoaderCircle className="spin" size={19} /><h3>正在阅读你选择的内容</h3><p>不会自动改动任何职位、材料或面经。</p></div> : state.error ? <div className="career-ai-error"><X size={24} /><h3>这次没有生成成功</h3><p>{state.error}</p><button className="career-button secondary" onClick={onClose}>返回手动编辑</button></div> : <><div className="career-ai-banner"><ShieldCheck size={17} /><span>建议来自配置的 DeepSeek 服务；个人经历仍需由你确认。</span></div><pre>{aiText(state.result)}</pre><div className="career-form-actions"><button className="career-button ghost" onClick={onClose}>放弃草稿</button><button className="career-button primary" onClick={() => void onAccept()}><Check size={16} />接受为草稿</button></div></>}</div></Modal>;
}
