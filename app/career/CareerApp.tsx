...
function AnalyticsView({ data }: { data: CareerData }) {
  ...
  const withNextStep = activeJobs.filter((job) => data.tasks.some((task) => task.job_id === job.id && task.status === "todo") || data.interviews.some((item) => item.job_id === job.id && item.status !== "completed"));
  ...
  const missingNextStep = activeJobs.filter((job) => !data.tasks.some((task) => task.job_id === job.id && task.status === "todo") && !data.interviews.some((item) => item.job_id === job.id && item.status !== "completed"));
