...
export default function FitnessApp() {
  ...
  return <main className="shilian">
    ...
    <section className="sl-workspace">
      ...
      />}
      {view === "venues" && <VenuesView snapshot={snapshot} venue={venue} onSelect={setVenueId} onAdd={() => { setEditingVenue(null); setDialog("venue"); }} onEditVenue={(entry) => { setEditingVenue(entry); setDialog("venue"); }} onArchive={(entry) => { if (!window.confirm(`归档「${entry.name}」吗？它的未来计划会停用，尚未开始的安排会取消；历史记录会保留。`)) return; void run(async () => { await archiveVenue(entry.id); }, `「${entry.name}」已归档，历史仍保留`); }} onAddEquipment={() => setDialog("equipment")} onEditEquipment={(entry) => { setEditingEquipment(entry); setDialog("equipment"); }} onStatus={(entry, status) => void run(async () => { await setEquipmentStatus(entry.id, status); }, status === "maintenance" ? "已标记为临时停用，未来计划会避开它" : "器材状态已更新")} />}
...
function MonthCalendar({ events }: { events: readonly FitnessCalendarEvent[] }) {
  ...
  return <div className="sl-month" aria-label={`${anchor.getFullYear()}年${anchor.getMonth() + 1}月`}><header>{weekdayNames.map((day) => <span key={day}>{day.slice(1)}</span>)}</header><div>{days.map((date) => { const begin = dayStart(date.getTime()); const rows = events.filter((event) => event.status !== "cancelled" && event.starts_at >= begin && event.starts_at < begin + 86_400_000); return <article key={date.toISOString()} className={date.getMonth() === anchor.getMonth() ? "" : "muted"}><time>{date.getDate()}</time>{rows.map((event) => <span key={event.id} className={event.kind}>{event.status === "completed" ? "已练" : event.status === "not_performed" ? "未进行" : event.kind === "cardio" ? "心肺" : event.kind === "rest" ? "休息" : "力量"}</span>)}</article>; })}</div></div>;
}
