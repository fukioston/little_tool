...
export function FitnessDialog({
  ...
  onKeepEditing,
  onDiscard,
  initialFocus = "[data-dialog-close]",
}: {
  open: boolean;
  title: string;
  eyebrow: string;
  ...
  const dialog = useFitnessDialog<HTMLElement>(open, requestClose, initialFocus);
  ...
  return <>
    ...
    <section ref={dialog} className={`sl-dialog ${wide ? "wide" : ""}`} role="dialog" aria-modal="true" aria-busy={busy} aria-labelledby="sl-dialog-title" tabIndex={-1}>
      ...
      {confirmClose && <section className="sl-dirty-close" role="alert" aria-live="polite"><div><b>保留还没保存的修改？</b><p>关闭会舍弃这张表单里尚未形成安全收据的内容。现有本地资料不会改变。</p></div><footer><button ref={keepEditing} type="button" onClick={onKeepEditing}>继续编辑</button><button type="button" className="sl-danger-action" onClick={onDiscard}>舍弃并关闭</button></footer></section>}
    </section>
  </>;
}
...
function useReportedFormDirty(onDirtyChange?: (dirty: boolean) => void) {
  ...
  useEffect(() => () => {
    if (dirty.current) callback.current?.(false);
  }, []);
  return useCallback(() => {
...
export function VenueForm({
  ...
  const markDirty = useReportedFormDirty(onDirtyChange);
  const [error, setError] = useState("");
  return <form className="sl-form" onChange={markDirty} onSubmit={(event) => {
    ...
    {writeBlocked && <p className="sl-form-hint" role="status">现有草稿会保留，但资料核对线索处理完成前不能保存。</p>}
    <FormBusyStatus busy={busy} />
    <footer><button type="button" disabled={busy} onClick={onClose}>取消</button><button className="sl-primary" disabled={busy || writeBlocked}>{busy ? "正在保存…" : "保存场地"}</button></footer>
  </form>;
}
...
export function EquipmentForm({
  ...
  return <form className="sl-form" onChange={markDirty} onSubmit={(event) => {
    ...
    <div className="sl-field-grid"><label><span>所在区域</span><input name="area" defaultValue={equipment?.area} placeholder="自由重量区" /></label><label><span>现在的状态</span><select name={equipment ? undefined : "status"} disabled={Boolean(equipment)} defaultValue={equipment?.status ?? "available"}><option value="available">可用</option><option value="limited">部分可用</option><option value="maintenance">临时停用</option><option value="removed">这里已没有</option></select>{equipment && <><input type="hidden" name="status" value={equipment.status} /><small>已有器材请从器材卡片单独修改状态。</small></>}</label></div>
    ...
    {writeBlocked && <p className="sl-form-hint" role="status">现有草稿会保留，但资料核对线索处理完成前不能保存。</p>}
    <FormBusyStatus busy={busy} />
    <footer><button type="button" disabled={busy} onClick={onClose}>取消</button><button className="sl-primary" disabled={busy || writeBlocked}>{busy ? "正在保存…" : "保存器材"}</button></footer>
  </form>;
}
...
export function ProfileForm({
  ...
  return <form className="sl-form" onChange={markDirty} onSubmit={(event) => {
    ...
    <fieldset className="sl-chip-picker"><legend>这段时间更在意什么？</legend>{goalOptions.map(([id, label]) => <button type="button" aria-pressed={goals.includes(id)} className={goals.includes(id) ? "active" : ""} key={id} onClick={() => { markDirty(); setGoals((current) => current.includes(id) ? current.filter((entry) => entry !== id) : [...current, id]); }}>{label}</button>)}</fieldset>
    ...
    <fieldset className="sl-chip-picker"><legend>通常方便训练的星期</legend>{weekdayNames.map((label, index) => <button type="button" aria-pressed={days.includes(index)} className={days.includes(index) ? "active" : ""} key={label} onClick={() => { markDirty(); setDays((current) => current.includes(index) ? current.filter((entry) => entry !== index) : [...current, index].sort()); }}>{label}</button>)}</fieldset>
    ...
    {writeBlocked && <p className="sl-form-hint" role="status">现有草稿会保留，但资料核对线索处理完成前不能保存。</p>}
    <FormBusyStatus busy={busy} />
    <footer><button type="button" disabled={busy} onClick={onClose}>取消</button><button className="sl-primary" disabled={busy || writeBlocked}>{busy ? "正在保存…" : "保存偏好"}</button></footer>
  </form>;
}
...
export function ConstraintForm({
  ...
  return <form className="sl-form" onChange={markDirty} onSubmit={(event) => {
    ...
    {writeBlocked && <p className="sl-form-hint" role="status">现有草稿会保留，但资料核对线索处理完成前不能保存。</p>}
    <FormBusyStatus busy={busy} />
    <footer><button type="button" disabled={busy} onClick={onClose}>取消</button><button className="sl-primary" disabled={busy || writeBlocked}>{busy ? "正在保存…" : "保存身体边界"}</button></footer>
  </form>;
}
