...
export function ContextPanel({ target, explanation, loading, error, showChinese, onChinese, onExplain, onSave, onClose }: { target: SelectionTarget; explanation: AiExplanation | null; loading: boolean; error: string; showChinese: boolean; onChinese: (value: boolean) => void | Promise<void>; onExplain: () => void; onSave: (note?: string) => void | Promise<void>; onClose: () => void }) {
  ...
  return <aside ref={dialog} className="sc-context-panel" role="dialog" aria-modal="true" aria-labelledby="sc-context-title" tabIndex={-1}><header><span id="sc-context-title">语境解释</span><button data-dialog-close onClick={onClose} aria-label="关闭语境解释">×</button></header><div className="sc-context-word"><div><h2>{explanation?.target?.canonical || target.surface}</h2><span>{pronunciation || "Selected in context"}</span></div><button className="sc-save-word" onClick={() => void onSave(note)}>＋ 收入词库</button></div>
    {!explanation && !loading && !error && <div className="sc-context-preview"><span>IN THIS CONTEXT</span><p>“{target.sentence}”</p><small>只发送所选内容与附近句子</small></div>}
    {loading && <div className="sc-ai-loading" role="status" aria-live="polite"><i/><span>正在理解这个语境…</span><small>不会发送整篇内容</small></div>}
    {error && <div className="sc-context-error" role="alert"><p>{error}</p><button onClick={onExplain}>重试</button></div>}
...
export function ImportWizard({ localLock, onClose, onImported }: { localLock: boolean; onClose: () => void; onImported: (id: string) => void | Promise<void> }) {
  ...
  const operation=useRef<AbortController|null>(null); const closed=useRef(false);
  const close=useCallback(()=>{closed.current=true;operation.current?.abort();onClose();},[onClose]);
  const dialog=useOverlayDialog<HTMLElement>(true,close,"button[data-dialog-close]");

  useEffect(()=>{
    const controller=new AbortController();
    void fetch("/api/health",{headers:{Accept:"application/json"},signal:controller.signal}).then(async(response)=>{
      const payload=await response.json() as {transcription?:{configured?:boolean}};
      if(!closed.current)setTranscriptionConfigured(Boolean(response.ok&&payload.transcription?.configured));
    }).catch(()=>{if(!closed.current&&!controller.signal.aborted)setTranscriptionConfigured(false);});
    return()=>{closed.current=true;controller.abort();operation.current?.abort();};
  },[]);

  const submit=async()=>{operation.current?.abort();const controller=new AbortController();operation.current=controller;setBusy(true);setError("");let localAudioKey:string|undefined;try{
    if(kind==="article"){
      let article;
      if(articleMode==="url"){
        if(localLock)throw new Error("本地锁已开启。可以改用粘贴或文件导入。");
        if(!/^https?:\/\//i.test(url))throw new Error("请输入完整的 http 或 https 地址");
        setStage(1);article=normalizeArticleApi(await postJson("/api/import/article",{url},controller.signal));article.sourceUrl ||= url;
      ...
      if(title.trim())article.title=title.trim();if(controller.signal.aborted)return;setStage(2);await onImported(await saveArticle(article,articleMode));
    }else if(kind==="rss"){
      if(localLock)throw new Error("本地锁已开启，无法访问 RSS。");
      if(!/^https?:\/\//i.test(url))throw new Error("请输入完整的 RSS 地址");
      setStage(1);const found=normalizePodcastApi(await postJson("/api/import/rss",{url},controller.signal));if(!found.length)throw new Error("订阅中没有找到可导入的单集");if(!controller.signal.aborted)setEpisodes(found);
    }else{
      ...
      if(transcribe&&audioFile&&segments.length===0){if(localLock)throw new Error("本地锁已开启，无法发送音频转写。");if(!transcriptionConfigured)throw new Error("尚未配置语音转写服务。可以直接导入 VTT、SRT、LRC 或纯文本字幕。");setStage(1);const form=new FormData();form.append("file",audioFile);form.append("language","en");const response=await fetch("/api/transcribe",{method:"POST",body:form,signal:controller.signal});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(String((payload as Record<string,unknown>).error??"转写失败"));const data=((payload as Record<string,unknown>).data??payload) as Record<string,unknown>;if(Array.isArray(data.segments))segments=data.segments.flatMap((entry,index)=>{if(!entry||typeof entry!=="object")return[];const row=entry as Record<string,unknown>;return[{start_ms:Number(row.start_ms??Number(row.start??index*5)*1000),end_ms:Number(row.end_ms??Number(row.end??(index+1)*5)*1000),text:String(row.text??""),speaker:typeof row.speaker==="string"?row.speaker:null}];});durationMs=Number(data.duration_ms??segments.at(-1)?.end_ms??0);}
      if(controller.signal.aborted)return;if(audioFile){setStage(2);const saved=await saveLocalFile("vocab",audioFile,{originalName:audioFile.name,mimeType:audioFile.type||"audio/mpeg",category:"podcast-audio"});localAudioKey=saved.key;if(controller.signal.aborted){await deleteLocalFile("vocab",localAudioKey).catch(()=>false);localAudioKey=undefined;return;}}
  ...
  }catch(caught){if(localAudioKey)await deleteLocalFile("vocab",localAudioKey).catch(()=>false);if(!closed.current&&!controller.signal.aborted){setError(errorMessage(caught));setStage(0);}}finally{if(operation.current===controller)operation.current=null;if(!closed.current)setBusy(false);}};

  const importEpisode=async(episode:ParsedPodcast)=>{operation.current?.abort();const controller=new AbortController();operation.current=controller;setBusy(true);setError("");try{let ready=episode;if(!episode.segments.length&&episode.transcriptUrl){try{const transcript=normalizeArticleApi(await postJson("/api/import/article",{url:episode.transcriptUrl},controller.signal));ready={...episode,segments:parseTranscript(transcript.blocks.map((block)=>block.text).join("\n\n"),episode.transcriptUrl)};}catch(error){if(controller.signal.aborted)throw error;/* The episode remains importable when its publisher blocks transcript fetching. */}}if(!controller.signal.aborted)await onImported(await savePodcast(ready,"rss"));}catch(caught){if(!closed.current&&!controller.signal.aborted)setError(errorMessage(caught));}finally{if(operation.current===controller)operation.current=null;if(!closed.current)setBusy(false);}};
  return <><button className="sc-modal-scrim" onClick={close} aria-label="关闭导入"/><section ref={dialog} className="sc-import-modal" role="dialog" aria-modal="true" aria-labelledby="sc-import-title" tabIndex={-1}><header><Logo/><div><span>添加到资料库</span><h2 id="sc-import-title">从哪里开始？</h2></div><button data-dialog-close onClick={close} aria-label="关闭导入内容">×</button></header><div className="sc-import-types">{([['article','英文文章'],['rss','RSS 播客'],['audio','音频 / 字幕']] as const).map(([id,label])=><button key={id} aria-pressed={kind===id} className={kind===id?"active":""} onClick={()=>{setKind(id);setError("");setEpisodes([]);}}>{label}</button>)}</div><div className="sc-import-body">
    {kind==="article"&&<><div className="sc-import-subtabs">{([['url','网页地址'],['paste','粘贴文本'],['file','本地文件']] as const).map(([id,label])=><button key={id} aria-pressed={articleMode===id} className={articleMode===id?"active":""} onClick={()=>setArticleMode(id)}>{label}</button>)}</div>{articleMode==="url"&&<label className="sc-field"><span>文章地址</span><input value={url} onChange={(event)=>setUrl(event.target.value)} placeholder="https://…"/><small>网页会经安全提取，只保留可读正文。</small></label>}{articleMode==="paste"&&<label className="sc-field"><span>英文文章内容</span><textarea value={paste} onChange={(event)=>setPaste(event.target.value)} placeholder="Paste an English article here…"/></label>}{articleMode==="file"&&<FileDrop file={articleFile} accept=".txt,.md,.html,text/plain,text/markdown,text/html" label="TXT、Markdown 或 HTML" onFile={setArticleFile}/>}<label className="sc-field compact"><span>标题（可选）</span><input value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="自动识别，也可以手动覆盖"/></label></>}
    ...
    {kind==="audio"&&<><FileDrop file={audioFile} accept="audio/*,.mp3,.m4a,.wav,.ogg" label="音频文件" onFile={setAudioFile}/><FileDrop file={transcriptFile} accept=".vtt,.srt,.lrc,.txt,text/vtt,text/plain" label="VTT、SRT、LRC 或纯文本字幕" onFile={(file)=>{setTranscriptFile(file);if(file)setTranscribe(false);}}/><label className="sc-field compact"><span>节目标题</span><input value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="Untitled podcast"/></label><label className="sc-check"><input aria-label="没有字幕时请求外部转写" type="checkbox" checked={transcribe} disabled={busy||Boolean(transcriptFile)||localLock||transcriptionConfigured!==true} onChange={(event)=>setTranscribe(event.target.checked)}/><span><b>没有字幕时，请求外部转写</b><small>{transcriptFile?"已选择字幕，不会上传音频转写。":localLock?"本地锁已开启，不会上传音频。":transcriptionConfigured===null?"正在检查是否配置了转写服务…":transcriptionConfigured?"只有音频会发送到已配置的转写端点。":"当前未配置转写服务；可以直接导入字幕文件。"}</small></span></label></>}
    {error&&<div className="sc-import-error" role="alert">{error}</div>}{busy&&<div className="sc-import-progress" role="status" aria-live="polite"><i><em style={{width:stage===1?"58%":stage===2?"90%":"22%"}}/></i><span>{stage===1?(kind==="audio"?"正在转写英文音频…":"正在读取远程内容…"):stage===2?"正在写入本地资料库…":"正在准备…"}</span></div>}</div>{!episodes.length&&<footer><button onClick={close}>取消</button><button className="sc-primary" disabled={busy} onClick={()=>void submit()}>{busy?"处理中…":kind==="rss"?"读取节目":"导入到拾词"}</button></footer>}</section></>;
}
