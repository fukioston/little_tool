...
export function ReviewView({ cards, onRate, onUndo }: { cards: ReviewCard[]; onRate: (card:ReviewCard,rating:ReviewRating)=>Promise<string>; onUndo:(id:string)=>Promise<void> }) {
  ...
  const total=reviewed+due.length; return <div className="sc-page sc-review-page"><header><div><span className="sc-eyebrow">BACK TO CONTEXT</span><h1>回到语境</h1></div><div><strong>{reviewed}</strong><span>/ {total}</span></div></header><div className="sc-review-progress"><i style={{width:`${reviewed/Math.max(1,total)*100}%`}}/></div><section className={`sc-review-card ${revealed?"revealed":""}`}><span className="sc-card-label">{revealed?"解释":"还记得这个语境吗？"}</span><h2>{card.headword}</h2><p className="sc-pronunciation">{revealed?card.pronunciation:""}</p><div className="sc-cloze">“{card.context_sentence?card.context_sentence.replace(new RegExp(escaped,"i"),"____"):`Recall “${card.headword}” in context.`}”</div>{!revealed?<button className="sc-reveal" onClick={()=>setRevealed(true)}>查看解释 <kbd>Space</kbd></button>:<div className="sc-answer"><strong>{card.gloss_en||"No definition yet"}</strong><p>{card.context_sentence}</p></div>}</section>{revealed&&<div className="sc-ratings">{([['again','再看一次','稍后','1'],['hard','有点难','更早','2'],['good','记得','按节奏','3'],['easy','很熟','更晚','4']] as const).map(([id,label,next,key])=><button key={id} className={id} disabled={busy} onClick={()=>void submit(id)}><span>{label}</span><small>{next}</small><kbd>{key}</kbd></button>)}</div>}<footer className="sc-review-footer"><span>{Math.max(0,due.length-1)} 个词可以继续回看</span>{lastEvent&&<button onClick={async()=>{await onUndo(lastEvent);setLastEvent(null);setReviewed((n)=>Math.max(0,n-1));}}>↶ 撤销上一次</button>}</footer></div>;
}
