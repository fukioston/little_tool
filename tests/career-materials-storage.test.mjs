...
function insertMaterial(database, {
  ...
  executeRun(
    ...
    `INSERT INTO career_materials(
     ...
     ) VALUES(?,?,'简历','v1','2026-08-21T00:00:00.000Z',NULL,?,'',?,NULL,NULL,NULL)`,
    [id, name, status, fileKey],
  );
}
...
test("material deletion source keeps every mutation guarded and behind the Career write lock", () => {
  ...
  assert.match(
    source,
    /UPDATE career_materials[\s\S]*WHERE id = \? AND status = \? AND file_key IS \?/,
  );
  assert.match(
    source,
    /DELETE FROM career_materials[\s\S]*WHERE id = \? AND status = 'deleting' AND file_key IS \?/,
  );
...
test("a legacy empty file key is no attachment and never reaches OPFS", async (t) => {
  ...
  assert.deepEqual(
    await context.service.inspectCareerMaterialDeletion("material-empty-key"),
    {
      state: "present",
  ...
  assert.deepEqual(
    await context.service.deleteCareerMaterial("material-empty-key"),
    {
      outcome: "deleted",
      materialId: "material-empty-key",
...
test("OPFS failure leaves a visible deleting row and an idempotent retry completes cleanup", async (t) => {
  ...
  const first = await context.service.deleteCareerMaterial("material-a");
  ...
  assert.deepEqual(
    await context.service.loadCareerMaterialDeletionState("material-a"),
    {
  ...
  const retried = await context.service.deleteCareerMaterial("material-a");
...
test("a shared file key is retained until the last material reference is removed", async (t) => {
  ...
  const last = await context.service.deleteCareerMaterial("material-b");
...
test("unknown ids are explicitly already absent and perform no cleanup", async (t) => {
  const context = await fixture({ files: [FILE_A] });
  t.after(() => context.close());

  assert.deepEqual(
    await context.service.deleteCareerMaterial("material-unknown"),
    { outcome: "already_absent", materialId: "material-unknown" },
  );
  assert.deepEqual(
...
test("double clicks are serialized: one deletes and the retry observes absence", async (t) => {
  ...
  const results = await Promise.all([
    context.service.deleteCareerMaterial("material-a"),
    context.service.deleteCareerMaterial("material-a"),
  ]);
  assert.deepEqual(results.map(({ outcome }) => outcome), ["deleted", "already_absent"]);
  assert.equal(context.state.lockCalls, 2);
...
test("a lost final SQLite response is verified as deleted instead of reported as failure", async (t) => {
  ...
  const result = await context.service.deleteCareerMaterial("material-a");
...
test("if final commit verification is also unavailable the result stays uncertain", async (t) => {
  ...
  assert.deepEqual(
    await context.service.deleteCareerMaterial("material-a"),
    {
